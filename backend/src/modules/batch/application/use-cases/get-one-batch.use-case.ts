...
export class GetOneBatchUseCase {
  ...
  async execute(id: UUID): Promise<GetBatchDto> {
    ...
    const models = await this.productModelRepository.findManyByIds(
      products.map((p) => p.modelId)
    )
    ...
    const items = products.map((p) => {
      ...
      item.modelName =
        modelNameObj ??
        new (Object.getPrototypeOf(item).constructor.ModelName)('')
      item.quantity = 1
      return item
    })
    ...
    const totalCostValue = products.reduce(
      (acc, p) => acc + Number(p.unitCost.getValue()),
      0
    )
    ...
    const supplier = batch.supplierId
      ? await this.supplierRepository.findById(batch.supplierId)
      : undefined
