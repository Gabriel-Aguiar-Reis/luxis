...
@Injectable()
export class ShipmentCaslRule implements CaslRuleBuilder {
  buildFor(user: UserPayload, builder: AbilityBuilder<AppAbility>) {
    const { can } = builder

    switch (user.role) {
      case Role.ADMIN:
        can(Actions.Manage, Shipment)
        break
      case Role.RESELLER:
        can(Actions.Read, Shipment, { resellerId: user.id })
        break
      case Role.ASSISTANT:
        can(Actions.Read, Shipment)
        break
    }
  }
}
